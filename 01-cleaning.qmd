...
4. Geocoding
5. Fix date data types and group by dates
6. Clean names in OpenRefine
...
```{r}
unhoused_merged <- bind_rows(unhoused_tidy_1, unhoused_tidy_2, unhoused_tidy_3, unhoused_tidy_4)
...
This shows both duplicates, so there are actually 89 (178/2) pairs of duplicates.
...
Proof it worked: subtract the number of duplicate pairs (89) from the total number of rows (30,611), that is how many rows are outputted below (30,522).

```{r}
unhoused_to_geocode <- unhoused_merged |>
...
unhoused_to_geocode |> glimpse()
```

## 4. Geocoding

### 4.1 Preparing to geocode

Reformatting addresses to prepare to geocode externally using geocodeio.

```{r}
unhoused_geo_prep <- unhoused_to_geocode |>
...
### 4.2 Exporting pre-geocode

Exporting the data to geocode in geocodio.
...
### 4.3 Importing geocoded

Importing the geocoded data from geocodio and cleaning column names.

```{r}
unhoused_geocoded <- read_csv("data-processed/unhoused-geocoded.csv") |> 
...
```{r}
unhoused_to_refine <- unhoused_geocoded |> 
...
```{r}
unhoused_data_types <- unhoused_refined |> #@ change this when refine/geocoding done
